...
public static class OrderEndpoints
{
    public static IEndpointRouteBuilder MapOrderEndpoints(this IEndpointRouteBuilder endpoints)
    {
        ...
        group.MapGet("/", async (int? page, int? pageSize, OrderStatus? status, ISender sender, CancellationToken cancellationToken) =>
        {
            var result = await sender.Send(
                new GetOrdersQuery(new PageParameters(page ?? 1, pageSize ?? 20), status),
                cancellationToken);
