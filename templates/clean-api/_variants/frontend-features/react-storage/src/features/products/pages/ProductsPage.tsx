...
export function ProductsPage({ user }: Props) {
  ...
  const save = useMutation({
    mutationFn: ({ product, model, image }: SaveProduct) => product
      ? productsApi.update(product.id, model, image)
      : productsApi.create(model, image),
  ...
  return (
    <Box component="section">
      ...
      <ProductFormDialog
        ...
        onSubmit={(model, image) => save.mutate({ product: editing, model, image })}
      />
      <ConfirmDialog
