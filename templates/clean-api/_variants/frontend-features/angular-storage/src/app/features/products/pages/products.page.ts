...
export class ProductsPage implements OnInit {
  ...
  async openForm(product: Product | null) {
    const result = await firstValueFrom(this.dialog.open<ProductFormDialogComponent, { product: Product | null }, ProductDialogResult>(ProductFormDialogComponent, { data: { product }, width: '620px', maxWidth: 'calc(100vw - 32px)', autoFocus: false }).afterClosed());
    if (!result) return;
    await this.saveProduct(product, result.model, result.image);
  }

  async saveProduct(product: Product | null, model: ProductModel, image: File | null) {
    try {
      if (product) await firstValueFrom(this.api.update(product.id, model, image));
      else await firstValueFrom(this.api.create(model, image));
