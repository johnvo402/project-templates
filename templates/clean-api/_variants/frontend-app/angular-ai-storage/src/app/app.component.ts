...
@Component({selector:'app-root',standalone:true,imports:[LoginPage,BusinessWorkspaceComponent,AppShellComponent,AiPage],template:`@if(!ready()){<main class="shell"><p>Restoring session…</p></main>}@else if(!auth.user()){<app-login-page (authenticated)="onAuthenticated($event)"/>}@else{<app-shell [user]="auth.user()!" (logout)="onLogout()"><app-business-workspace/>@if(auth.can('ai.generate')){<app-ai-page/>}</app-shell>}`})
