...
@Component({selector:'app-root',standalone:true,imports:[LoginPage,ProfilePage,TodoPage,UsersPage,AppShellComponent, AiPage],template:`@if(!ready()){<main class="shell"><p>Restoring session…</p></main>}@else if(!auth.user()){<app-login-page (authenticated)="onAuthenticated($event)"/>}@else{<app-shell [user]="auth.user()!" (logout)="onLogout()"><app-profile-page/>@if(auth.can('users.read')){<app-users-page/>}@if(auth.can('todos.read')){<app-todo-page/>}@if(auth.can('ai.generate')){<app-ai-page/>}</app-shell>}`})
