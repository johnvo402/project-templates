...
export function App() {
  ...
  return <AuthenticatedApp user={user} onLogout={() => void logout().finally(() => setUser(null))} onProfileUpdated={displayName => setUser(current => current ? { ...current, displayName } : current)} productImagesPage={<ProductImagesWorkspace user={user} />} aiPage={<AiPanel />} />;
}
