...
public sealed class AppUser : AggregateRoot<UserId>
{
    ...
    private AppUser(UserId id, string email, string normalizedEmail, string passwordHash, string role, string displayName)
        : base(id)
    {
        Email = email;
    ...
    public string? AvatarObjectName { get; private set; }
    public bool IsActive { get; private set; }
    public DateTimeOffset CreatedAtUtc { get; private set; }

    public static AppUser Create(string email, string normalizedEmail, string passwordHash, string role, string displayName)
    {
    ...
    {
        if (string.IsNullOrWhiteSpace(objectName)) throw new DomainException("Avatar object name is required.");
