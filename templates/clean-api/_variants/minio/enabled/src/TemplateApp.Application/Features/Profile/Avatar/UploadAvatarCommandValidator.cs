...
public sealed class UploadAvatarCommandValidator : AbstractValidator<UploadAvatarCommand>
{
    ...
    public UploadAvatarCommandValidator()
    {
        ...
        RuleFor(command => command.Model.FileName).NotEmpty().MaximumLength(255);
        RuleFor(command => command.Model.ContentType).Must(type => AllowedTypes.Contains(type, StringComparer.OrdinalIgnoreCase))
            .WithMessage("Avatar must be JPEG, PNG, or WebP.");
        RuleFor(command => command.Model.Content)
            .NotEmpty()
            .Must(content => content.Length <= MaxBytes)
            .WithMessage("Avatar must be 5 MB or smaller.")
            .Must((command, content) => HasValidSignature(command.Model.ContentType, content))
