...
export function TodoPage({ canWrite }: Props) {
  ...
  const todos = useTodos({ page, pageSize });
  ...
  return <section className="card span-2">
    ...
    {canWrite && <form className="inline-form" onSubmit={submit}><input placeholder="What needs to be done?" value={title} onChange={event => setTitle(event.target.value)} /><button className="primary" disabled={createTodo.isPending}>Add task</button></form>}
