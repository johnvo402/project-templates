...
@Component({
  ...
  `],
  template: `
    <app-page-header eyebrow="Sales" title="Orders" description="Create orders and move them through fulfillment.">
      <div pageActions><button mat-stroked-button (click)="load()" [disabled]="loading()">Refresh</button>@if(auth.can('orders.create')){<button mat-flat-button (click)="openForm()" [disabled]="loadingProducts()">{{loadingProducts()?'Loading…':'New order'}}</button>}</div>
    </app-page-header>
    ...
    @if(page(); as result){
      @if(result.data.length){
        <app-order-table [orders]="result.data" [busy]="actionBusy()" [canUpdate]="auth.can('orders.update-status')" [canCancel]="auth.can('orders.cancel')" (changeStatus)="updateStatus($event.order,$event.status)" (cancel)="cancelOrder($event)"/>
...
export class OrdersPage implements OnInit {
  ...
  applyFilters(filters: OrderFilterState) {
    ...
    this.query = { ...this.query, page: 1, keyword: filters.keyword.trim() || undefined, targets: ['OrderNumber', 'CustomerName', 'CustomerPhone'], sort: filters.sort || undefined, filters: lhs };
    void this.load();
  }
  ...
  async createOrder(model: CreateOrderModel) {
    this.actionBusy.set(true);
    try { await firstValueFrom(this.api.create(model)); this.notifications.success('Order created.'); await this.load(); }
    catch (error) { this.notifications.error(getErrorMessage(error)); }
    finally { this.actionBusy.set(false); }
  }

  async updateStatus(order: Order, status: MutableOrderStatus) {
    this.actionBusy.set(true);
    try { await firstValueFrom(this.api.updateStatus(order.id, status)); this.notifications.success(`Order moved to ${status}.`); await this.load(); }
    catch (error) { this.notifications.error(getErrorMessage(error)); }
    finally { this.actionBusy.set(false); }
  }

  async cancelOrder(order: Order) {
    const confirmed = await this.confirm.confirm({ title: 'Cancel order?', description: `Cancel ${order.orderNumber}? Reserved stock will be restored.`, confirmLabel: 'Cancel order', destructive: true });
    if (!confirmed) return;
    this.actionBusy.set(true);
    try { await firstValueFrom(this.api.cancel(order.id)); this.notifications.success('Order cancelled.'); await this.load(); }
    catch (error) { this.notifications.error(getErrorMessage(error)); }
    finally { this.actionBusy.set(false); }
  }
}
